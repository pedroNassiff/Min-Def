...
const Monitoreos = () => {
    ...
    useEffect(() => {
      // trayendo Acciones
      getData(`${domain}/wp-json/wp/v2/acciones?filter[meta_key]=area&filter[meta_value]=monitoreos`);
    }, []);

    const searchFilter = (search) => {
      getData(`${domain}/wp-json/wp/v2/acciones?filter[meta_key]=area&filter[meta_value]=monitoreos&search=${search}`);
    }

    return (
        <div className="bibliotecaContainer">
            <div className="titleContainer">
                <h1>Acciones - Monitoreos</h1>
                <span>
