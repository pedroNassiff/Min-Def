...
const Informes = () => {
    ...
    useEffect(() => {
      // trayendo Acciones
      getData(`${domain}/wp-json/wp/v2/acciones?filter[meta_key]=area&filter[meta_value]=informes`);
    }, []);

    const searchFilter = (search) => {
      getData(`${domain}/wp-json/wp/v2/acciones?filter[meta_key]=area&filter[meta_value]=informes&search=${search}`);
    }

    return (
        <div className="bibliotecaContainer">
            <div className="titleContainer">
                <h1>Acciones - Informes</h1>
                <span>
